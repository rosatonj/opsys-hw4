...
/* Note: debug stack areas are now defined in tsystm.h */
...
/* Note: MAX_CALL is now defined in tsyscall.h  */
...
void k_init(){
    init_debuglog();	/* specify debug logging area in memory */
    cli();
    system_time = 0;
    init_ticks();
...
/****************************************************************************/
/* syssleep: this function for the sleep syscall function */
/****************************************************************************/
...
{ 
  ...
  sprintf(buf,"SL%d",curproc-proctab);
  debuglog(buf);
#endif
  cli();
  curproc->p_wakeup_time = system_time + ms;
  tsleep(TIME_UP);
  return 0;   
}
