...
/* Note: don't expand the kprintf here--if it's too large, it slows
   down the interrupt sequence. One char takes only 1 ms to output,
   so that's OK.  Add any debuglog entries you want.
*/
void tick_handler()
{
    char buf[100];
    ...
    for (i = 1; i < NPROC; i++) /* check for processes to wake up */
	if (proctab[i].p_status == BLOCKED
	    && proctab[i].p_waitcode == TIME_UP
	    && system_time >= proctab[i].p_wakeup_time) {
	    /* sprintf(buf,"WU%d", i);  report wakeup to debuglog*/
	    /* debuglog(buf); */
	    twakeup_one(i);	/* set its p_status to RUN */
	}
}
